...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time)
{
    ...
    start3 = ((double)start_s3.tv_sec + (double)((start_s3.tv_nsec)/(double)1000000000)); //time in seconds
    syslog(LOG_CRIT," writeback start Time: %lf seconds\n",start3);

    int written, i, total, dumpfd;
    
    snprintf(&ppm_dumpname[11], 9, "%04d", dump_count);
    ...
    //         (long)time->tv_sec, (long)(time->tv_nsec / 1000000), HRES_STR, VRES_STR);
    
    // strncat(ppm_header, "# ", 2);  // Add a comment line for uname output
    // strncat(ppm_header, system_info.release, sizeof(system_info.release));
    // strncat(ppm_header, "\n", 1);
...
void main(int argc, char **argv)
{
    ...
    threadParams[0].sequencePeriods=56000;
...
void *Service_3_writeback(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        if ((dump_count<=1801)&&(dump_correct>2))
        {
